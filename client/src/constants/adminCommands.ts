...
export const ADMIN_COMMAND_CATEGORIES: AdminCommandCategory[] = [
  ...
  },
  {
    ...
    commands: [
      ...
      {
        id: 'add-admin',
        label: 'Add Server Admin (@css/root)',
        command: 'add-admin',
        description: 'Grant CSS root admin to a Steam ID on the server (css_addadmin). Takes effect until map change or server restart. Use Reload Admins to persist from admins.json.',
        requiresInput: true,
        inputLabel: 'Steam ID64 (e.g. 76561198012345678)',
        inputType: 'text',
        color: 'warning',
      },
      {
        id: 'remove-admin',
        label: 'Remove Server Admin',
        command: 'remove-admin',
        description: 'Remove CSS admin from a Steam ID on the server (css_removeadmin).',
        requiresInput: true,
        inputLabel: 'Steam ID64 (e.g. 76561198012345678)',
        inputType: 'text',
        color: 'error',
